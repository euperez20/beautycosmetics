ActiveAdmin.register Product do
  ...
  form do |f|
    f.inputs 'Product Details' do
      ...
      f.input :brand
      f.input :category
      f.input :image, as: :file, hint: f.object.image.attached? ? image_tag(f.object.image.variant(resize: '100x100')) : content_tag(:span, "No image yet")
      # f.input :colors, as: :check_boxes, collection: Color.all
    end
    f.actions do
      f.action :submit
      f.cancel_link { |resource| admin_product_path(resource) }
    end
  end
