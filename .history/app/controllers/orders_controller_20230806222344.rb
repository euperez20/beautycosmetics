class OrdersController < ApplicationController
  ...
  before_action :set_order, only: %i[show edit update destroy update_partial create]
  ...
  def create
    ...
    # Crear la orden y asociarla con el usuario actual (si está autenticado)
    # Si el usuario no está autenticado, simplemente crearemos la orden sin asociarla con un usuario
    province = Province.find_by(id: province_id)
    order = if user_signed_in?
              current_user.orders.new(
                name: user_name,
                address: user_address,
                province: province, # Cambiar 'Province.find_by(id: province_id)&.name' por 'province'
              ...
              puts "Creating order with user_name: #{user_name}, address: #{user_address}, province_id: #{province_id}"
