class CartItemsController < ApplicationController
  ...
  def show
    ...
    # Obtener el ID del item del carrito que deseas mostrar (puede ser pasado como parámetro en la URL, por ejemplo).
    item_id = params[:id].to_i

    # Utilizar el ID del item del carrito para encontrar el elemento específico en el carrito.
    @cart_item = @cart_items.find { |item| item['product_id'].to_i == item_id }

    # Verificar si se encontró el elemento del carrito.
    # Si no se encontró, puedes manejarlo de la manera que prefieras, por ejemplo, redirigiendo a una página de error o mostrando un mensaje de error.
    if @cart_item.nil?
      flash[:alert] = 'Elemento del carrito no encontrado.'
      redirect_to cart_items_path
      return
    end

    # Lógica adicional para mostrar los detalles del elemento del carrito en la vista.
  end
