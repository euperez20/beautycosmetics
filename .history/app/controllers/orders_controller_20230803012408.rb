class OrdersController < ApplicationController
  before_action :set_order, only: %i[ Search through the available products using a keyword search by category. 
    - User should be able to search for products by keyword.
    - Found products will contain the keyword somewhere in the product title or description.
    - When searching the user will be able to select a category (or other relevant association) from a dropdown list.
    - If a category is selected the search results will be restricted to products belonging to the selected category.
    - There should still be a way to search through all products regardless of category.
    - Your solution to this requirement should be different from your solution to feature 2.2 (navigate by category).
    edit update destroy update_partial]
