class ProductsController < ApplicationController
  ...
  def index
    ...
    # Búsqueda por nombre de producto y marca dentro de la categoría seleccionada
    if params[:search].present?
      search_term = "%#{params[:search]}%"
      @products = @products.where("UPPER(name) LIKE ? OR UPPER(brand) LIKE ?", search_term.upcase, search_term.upcase)
    end
  
    # Paginar la colección de productos con Kaminari
    @products = @products.page(params[:page]).per(12)
  
    # Guardar en un array si no hay productos para la categoría y la búsqueda
