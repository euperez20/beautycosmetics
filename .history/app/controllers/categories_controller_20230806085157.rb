class CategoriesController < ApplicationController
  ...
  def search
    ...
    if category_id.present? && category_id != 'All Categories'
      @category = Category.find(category_id)
      @products = @category.products.where('name LIKE ?', "%#{search_term}%")
    else
      @products = Product.where('name LIKE ?', "%#{search_term}%")
    end
    @results = perform_search(params[:category_id], params[:search])

    respond_to do |format|
      format.html { redirect_to root_path } # Puedes redirigir a la página de inicio o a otra página
      format.json { render json: @results } # Si necesitas una respuesta JSON
    end
  end
