class CartItemsController < ApplicationController
  ...
  def checkout
    ...
    if params[:user_name].blank? || params[:user_address].blank? || params[:province_id].blank?
      flash[:alert] = 'Please provide your name, address, and select a valid province.'
      redirect_to cart_items_path
      return
    end

    # Almacenar los valores del usuario en la sesión.
    session[:user_name] = params[:user_name]
    session[:user_address] = params[:user_address]
    session[:province_id] = params[:province_id]

    # Redirigir a la acción show (checkout) para mostrar los detalles del carrito y el total después de impuestos.
  ...
  def show
    ...
    @province_name = session[:province_name]
