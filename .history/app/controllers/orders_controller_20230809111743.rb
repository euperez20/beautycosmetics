class OrdersController < ApplicationController
  ...
  def show
    if @order.nil?
      flash[:error] = "Order not found." # Mensaje de error
      redirect_to orders_path # Redirige a la lista de órdenes u otra página
    else
      render 'orders/show'
    end
  end
  ...
  def search
    if params[:email].present? && params[:order_number].present?
      @order = Order.find_by(email: params[:email], order_number: params[:order_number])
    end
  end
